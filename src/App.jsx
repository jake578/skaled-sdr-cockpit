            ...
            {pipelineTab === "closedWon" && (
              <div>
                ...
                {closedWonOpps?.map(opp => {
                  const cwExpanded = expandedAction === `cw-${opp.id}`;
                  return (
                    <div key={opp.id} style={{
                      background: "#1E293B", borderRadius: 8, padding: "14px 16px", marginBottom: 8,
                      border: cwExpanded ? "1px solid #10B981" : "1px solid #334155",
                      borderLeft: "3px solid #10B981", cursor: "pointer",
                    }}
                    onClick={() => setExpandedAction(cwExpanded ? null : `cw-${opp.id}`)}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 700, color: "#F1F5F9" }}>{opp.name}</div>
                      ...
                      {cwExpanded && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #334155", display: "flex", gap: 6, flexWrap: "wrap" }} onClick={e => e.stopPropagation()}>
                          <a href={`https://skaled.my.salesforce.com/${opp.id}`} target="_blank" rel="noreferrer" style={{ padding: "6px 14px", borderRadius: 6, fontSize: 12, fontWeight: 600, background: "#00A1E0", color: "#fff", textDecoration: "none" }}>Open in SFDC</a>
                          <button style={s.btn("#10B981")} onClick={() => setShowDealInspector({ oppId: opp.id, oppName: opp.name })}>Inspect</button>
                          <button style={s.btn("#8B5CF6")} onClick={() => setShowDealScore({ oppId: opp.id, oppName: opp.name })}>Score</button>
                          <button style={{ padding: "6px 14px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: "linear-gradient(135deg, #8B5CF6, #EC4899)", color: "#fff" }} onClick={() => setShowDeepIntel({ oppId: opp.id, oppName: opp.name, accountName: opp.account })}>Deep Intel</button>
